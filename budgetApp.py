...
def api_analytics_velocity():
    ...
    expense_events = db.session.query(EventLog).filter(EventLog.event_type == 'TRANSACTION_CREATED').all()
    ...
    goal_events = db.session.query(EventLog).filter(EventLog.event_type == 'BUDGET_GOAL_SET').order_by(EventLog.timestamp.asc()).all()
    ...
    all_stored_goals = db.session.query(BudgetGoal).all()
    for goal in all_stored_goals:
        if goal.category not in active_budget_goals:
            active_budget_goals[goal.category] = goal.monthly_limit
